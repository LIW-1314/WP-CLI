...
from wpcli.agent import Agent
from wpcli.config import LlmConfig, WpCliConfig, load_config
from wpcli.llm import calculate_cost, create_llm_client, get_builtin_price_profile
from wpcli.llm.openai_compatible import OpenAICompatibleClient
...
def test_agent_propagates_connection_error_without_raising(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("wpcli.agent.agent.SnapshotService", NoopSnapshotService)
    monkeypatch.setattr(httpx.AsyncClient, "stream", fail_stream)
    config = WpCliConfig()
...
def test_unknown_model_context_and_prices_can_be_configured(tmp_path) -> None:
    project_config = tmp_path / ".wpcli" / "config.json"
