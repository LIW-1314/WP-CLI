...
def test_config_precedence(tmp_path, monkeypatch):
    ...
    (home / ".wpcli").mkdir(parents=True)
    (project / ".wpcli").mkdir(parents=True)
    (home / ".wpcli" / "config.json").write_text(
        json.dumps({"llm": {"provider": "home", "model": "home-model"}}),
        encoding="utf-8",
    )
    (project / ".wpcli" / "config.json").write_text(
        json.dumps({"llm": {"provider": "project", "model": "project-model"}}),
        encoding="utf-8",
    )
    (project / ".env").write_text("WPCLI_MODEL=env-file-model\n", encoding="utf-8")
    monkeypatch.setenv("HOME", str(home))
    monkeypatch.setenv("WPCLI_PROVIDER", "process")
...
def test_provider_specific_api_key(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("WPCLI_PROVIDER", "deepseek")
    monkeypatch.delenv("WPCLI_API_KEY", raising=False)
