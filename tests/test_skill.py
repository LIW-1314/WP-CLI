...
from wpcli.skill import SkillContextBuffer, SkillMatcher, SkillRegistry, SkillStateStore
from wpcli.tools import ToolRegistry, get_builtin_tools
from wpcli.tools.base import ToolContext
from wpcli.tools.builtins import load_skill
...
def test_skill_registry_layers_and_state(tmp_path, monkeypatch):
    ...
    _write_skill(project / ".wpcli" / "skills", "project-only", "project desc", "v2")
...
def test_load_skill_pushes_body_into_context_buffer(tmp_path, monkeypatch):
    monkeypatch.setenv("HOME", str(tmp_path / "home"))
    _write_skill(tmp_path / ".wpcli" / "skills", "demo", "demo desc", "v1", body="demo body")
...
def test_skill_registry_create_update_and_scope(tmp_path, monkeypatch):
    ...
    assert created.path == project / ".wpcli" / "skills" / "release-check" / "SKILL.md"
...
def test_save_skill_tool_requires_approval_and_persists_after_approval(tmp_path, monkeypatch):
    ...
    assert not (tmp_path / ".wpcli" / "skills" / "test-workflow" / "SKILL.md").exists()
    ...
    assert (tmp_path / ".wpcli" / "skills" / "test-workflow" / "SKILL.md").is_file()
