...
- WpCLI 自身也可以作为 MCP server 暴露内置工具
...
```bash
git clone https://github.com/itwanger/WpCLI-Python.git
cd WpCLI-Python
uv sync --extra dev
uv run wpcli --help
```
...
```bash
uv run wpcli -p "帮我总结这个项目"
```

选择运行模式并输出机器可读的 usage/cost：

```bash
uv run wpcli --mode plan -p "先读取 README，再验证项目" --json
uv run wpcli --mode team --worker-mode plan -p "并行审计核心模块" --json
```
...
```dotenv
WPCLI_PROVIDER=deepseek
WPCLI_MODEL=deepseek-v4-flash
...
```bash
uv run wpcli --provider deepseek --model deepseek-v4-flash
```

连接本地 OpenAI-compatible 服务：

```bash
WPCLI_PROVIDER=openai-compatible \
WPCLI_BASE_URL=http://127.0.0.1:11434/v1 \
WPCLI_MODEL=qwen2.5-coder \
uv run wpcli -p "解释这个仓库"
```

## 交互命令

进入 `uv run wpcli` 后，可以使用这些 slash commands：
...
保存在权限为 `0600` 的 `~/.wpcli/models.json`；建议填写 API Key 环境变量名，只有显式输入
API Key 时才会把密钥写入该文件。

## 内置工具

WpCLI 内置了一组 Agent 可以调用的本地工具和联网工具：
...
- builtin：产品默认能力
- user：`~/.wpcli/skills/*/SKILL.md`，跨项目复用
- project：`.wpcli/skills/*/SKILL.md`，最贴近当前仓库并拥有最高优先级
...
- 静态长期记忆：`AGENTS.md`、`PAI.md`、`.wpcli/PAI.md` 及自定义 prompt 文件；人工维护、可版本控制
...
WpCLI 可以连接 MCP server，并把远端工具动态注册为：
...
```bash
uv run wpcli mcp init-chrome --scope project
```

它会写入 `.wpcli/mcp.json`，内容类似：
...
```bash
uv run wpcli mcp list
```

把 WpCLI 自身作为 MCP server 暴露：

```bash
uv run wpcli mcp serve --transport stdio
uv run wpcli mcp serve --transport http --port 3000
```
...
WpCLI 内置轻量 Runtime API，适合外部系统接入线程、turn、事件和后台任务。

启动服务：

```bash
WPCLI_RUNTIME_API_KEY=dev-key \
uv run wpcli serve --http --port 8080
```
...
```bash
uv run wpcli worker --workers 2 --cwd .
```
...
本地图片会自动压缩、缩放，并在需要时把透明底铺成白底，再转为 data URL。如果当前 provider/model 不支持多模态输入，WpCLI 会自动降级为文本元信息，不会把不支持的图片 payload 发给模型。
...
快照保存在 `~/.wpcli/snapshots/`，不会写入项目 `.git`。
...
```python
from wpcli.sdk import create_default_engine
