"""cli.py — WpCLI terminal AI agent CLI entry point.
...
Usage modes::
    ...
    wpcli
    wpcli --api-key sk-xxx
    wpcli --model deepseek-chat --provider deepseek

    # Single-shot prompt
    wpcli -p "list all Python files"
    wpcli -p "refactor this module" --mode plan

    # Tooling
    wpcli doctor              # system health check
    wpcli serve               # Runtime HTTP API
    wpcli mcp serve           # Expose tools via MCP
    wpcli mcp init-chrome     # Configure Chrome DevTools MCP
"""
...
from wpcli.config import get_config_paths, load_config
from wpcli.entrypoints.repl import start_repl
from wpcli.llm import create_llm_client
from wpcli.mcp import load_mcp_server_specs, serve_http, serve_stdio, write_chrome_devtools_config
from wpcli.runtime import RuntimeApiServer
from wpcli.runtime.api import runtime_api_key

app = typer.Typer(
    name="wpcli",
    help="WpCLI — Terminal AI Agent in Python (Claude Code alternative)",
...
def _version_callback(value: bool) -> None:
    if value:
        typer.echo(f"wpcli {__version__}")
        raise typer.Exit()
...
def main(
    ...
    """WpCLI — a terminal AI agent that works in your workspace.
...
def runtime_serve(
    ...
    api_key: Annotated[
        str | None,
        typer.Option("--api-key", help="Runtime API key. Defaults to WPCLI_RUNTIME_API_KEY."),
    ] = None,
    cwd: Annotated[Path | None, typer.Option("--cwd", help="Working directory")] = None,
) -> None:
...
def mcp_serve(
    ...
    """Expose WpCLI tools via the Model Context Protocol."""
    ...
    *,
    ...
    if not config.llm.api_key:
        typer.echo(
            ...
            "via the WPCLI_API_KEY environment variable, or in "
            "~/.wpcli/config.json or .wpcli/config.json.",
            err=True,
        )
        raise typer.Exit(1)
