...
from wpcli import __version__
from wpcli.agent import Agent, AgentOrchestrator, PlanExecuteAgent
from wpcli.bootstrap import build_tool_registry
from wpcli.config import WpCliConfig, config_to_public_dict
from wpcli.entrypoints.model_selector import ModelSelectorState, run_model_selector
from wpcli.llm import create_llm_client
from wpcli.llm.model_profiles import (
    DEFAULT_MODEL_PROFILES,
    PROVIDER_DEFAULTS,
    CustomModelStore,
    ModelProfile,
)
from wpcli.memory import MemoryManager
from wpcli.policy import AuditLog
from wpcli.prompt import PromptAssembler
from wpcli.rag import CodeIndex
from wpcli.render import RichRenderer
from wpcli.runtime import DurableTaskManager
from wpcli.skill import SkillRegistry
from wpcli.snapshot import SnapshotService
from wpcli.tools import ToolRegistry
...
async def start_repl(cwd: str, config: WpCliConfig) -> None:
    console = Console()
    ...
    history_path = Path.home() / ".wpcli" / "history" / "prompt_history.txt"
...
async def _handle_slash(
    ...
    if command == "/help":
        ...
        table = Table(title="WpCLI Context")
        ...
        console.print("Use `wpcli mcp serve --transport stdio|http --port 3000` to expose tools.")
    elif command == "/task":
...
async def _memory_command(arg: str, console: Console, cwd: str, config: WpCliConfig) -> None:
...
def config_base_url(provider: str) -> str:
    from wpcli.llm.factory import DEEPSEEK_BASE_URL, OPENAI_BASE_URL, PROVIDER_BASE_URLS
...
def _provider_api_key_env(provider: str) -> str:
    return {
        ...
        "openai-compatible": "WPCLI_API_KEY",
    }.get(provider.lower(), "WPCLI_API_KEY")
...
def _task_command(arg: str, console: Console, cwd: str) -> None:
    manager = DurableTaskManager(Path.home() / ".wpcli" / "tasks" / "tasks.db", scope=cwd)
    sub, _, rest = arg.partition(" ")
    if sub == "add" and rest:
        try:
        ...
        console.print(
            f"Queued {task_id} ({mode}). Run `wpcli worker` or `wpcli serve` to consume it."
        )
