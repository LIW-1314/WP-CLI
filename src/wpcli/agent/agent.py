...
from wpcli.agent.orchestrator import AgentOrchestrator
from wpcli.agent.plan_execute import PlanExecuteAgent
from wpcli.agent.query import query as _query
from wpcli.config import WpCliConfig
from wpcli.context import ContextBudget, ContextWindowManager
from wpcli.image import parse_image_references
from wpcli.llm.base import LlmClient
from wpcli.llm.pricing import CostBreakdown, ModelPriceProfile
from wpcli.prompt import PromptAssembler
from wpcli.skill import SkillContextBuffer, SkillRegistry
from wpcli.snapshot import SnapshotService
from wpcli.tools.base import ToolContext
from wpcli.tools.executor import ToolExecutor
from wpcli.tools.registry import ToolRegistry
from wpcli.types import Message, QueryResult, Usage
    ...
    def _validate_config(self) -> None:
        ...
        if not self.config.llm.api_key:
            raise ValueError(
                ...
                "Set WPCLI_API_KEY (or DEEPSEEK_API_KEY / GLM_API_KEY / etc.) "
                "in the environment or in ~/.wpcli/config.json."
            )
        if not self.llm_client.max_context_window:
            raise ValueError(
                "LLM context window is not configured. "
                "Set a WPCLI_CONTEXT_WINDOW environment variable or configure "
                "it in ~/.wpcli/config.json."
            )
...
def _prepend_skill_candidates(user_message: str, cwd: str, config: WpCliConfig) -> str:
