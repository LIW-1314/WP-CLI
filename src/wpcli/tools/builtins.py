...
from wpcli.policy import CommandGuard
from wpcli.rag import CodeIndex
from wpcli.skill import SkillRegistry
from wpcli.snapshot import SnapshotService
from wpcli.tools import file_ops as fops
from wpcli.tools.base import Tool, ToolContext, ToolResult, object_schema
from wpcli.tools.file_ops import FileOpResult
from wpcli.web import fetch_url, search_web
...
def get_builtin_tools() -> list[Tool]:
    tools = [
        ...
        Tool(
            name="load_skill",
            description="Load a named WpCLI skill manual from user/project skill directories.",
            parameters=object_schema(
        ...
        Tool(
            name="save_skill",
            description=(
                "Persist a reusable WpCLI workflow as a project or user skill after user approval."
            ),
            parameters=object_schema(
                {
        ...
        Tool(
            name="revert_turn",
            description="Restore the workspace to a previous WpCLI side-history snapshot.",
            parameters=object_schema(
