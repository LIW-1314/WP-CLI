...
class MemoryConfig:
    ...
    long_term_db_path: str = "~/.wpcli/memory.db"
...
class PolicyConfig:
    ...
    audit_log_path: str = "~/.wpcli/audit.jsonl"
...
def load_config(
    ...
    data = _config_to_dict(WpCliConfig())

    user_config = _read_json(_home() / ".wpcli" / "config.json")
    if user_config:
        data = _deep_merge(data, user_config)

    root = Path(project_root).resolve() if project_root else None
    if root:
        project_config = _read_json(root / ".wpcli" / "config.json")
        if project_config:
...
def get_config_paths(project_root: str | Path | None = None) -> list[Path]:
    paths = [_home() / ".wpcli" / "config.json"]
    if project_root:
        paths.append(Path(project_root).resolve() / ".wpcli" / "config.json")
    return paths


def config_to_public_dict(config: WpCliConfig) -> dict[str, Any]:
...
def _apply_env(data: dict[str, Any], env: dict[str, str | None]) -> dict[str, Any]:
    ...
    mappings: list[tuple[str, str, Any]] = [
        ("WPCLI_API_KEY", "api_key", str),
        ("WPCLI_PROVIDER", "provider", str),
        ("WPCLI_MODEL", "model", str),
        ("WPCLI_BASE_URL", "base_url", str),
        ("WPCLI_CONTEXT_WINDOW", "context_window", int),
        ("WPCLI_MAX_TOKENS", "max_tokens", int),
        ("WPCLI_TEMPERATURE", "temperature", float),
    ]
    ...
    render_mode = env.get("WPCLI_RENDER_MODE") or env.get("WPCLI_RENDERER")
    if render_mode in {"plain", "inline"}:
        result["render_mode"] = render_mode

    if env.get("WPCLI_TUI") == "true":
...
def _config_to_dict(config: WpCliConfig) -> dict[str, Any]:
    return asdict(config)


def _dict_to_config(data: dict[str, Any]) -> WpCliConfig:
    return WpCliConfig(
