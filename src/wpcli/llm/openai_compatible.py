...
from wpcli.llm.pricing import CostBreakdown, ModelPriceProfile, calculate_cost
        ...
        *,
        ...
        if self.price_profile is None:
            raise ValueError(
                ...
                "Set llm.prices in WpCLI config."
            )
        ...
        *,
        ...
        if not self.api_key:
            yield {
                "type": "error",
                "error": RuntimeError(
                    "WPCLI_API_KEY is not configured. Set it in env, ~/.wpcli/config.json, "
                    "or project .wpcli/config.json."
                ),
            }
            return
